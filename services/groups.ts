...
function looksLikeContributionOrPaymentRow(value: any): boolean {
  ...
  const hasPaymentFields =
    ...
    value.mpesa_receipt_number !== undefined ||
    value.transaction_date !== undefined ||
    value.ledger_reference !== undefined ||
    value.total_contributed !== undefined ||
    value.my_share !== undefined ||
    value.fund !== undefined;

  const hasMembershipFields =
    value.role !== undefined ||
    value.is_active !== undefined ||
    value.joined_at !== undefined ||
    value.group_name !== undefined;

  return hasPaymentFields && !hasMembershipFields;
}

function isGroupMembershipRow(value: any): value is GroupMembership {
  if (!value || typeof value !== "object") return false;
  if (looksLikeContributionOrPaymentRow(value)) return false;

  const groupValue = value.group;
  const groupId =
    toOptionalNumber(groupValue) ||
    toOptionalNumber(groupValue?.id) ||
    toOptionalNumber(value.group_id);

  if (!groupId) return false;

  // A real membership row should carry at least one membership signal.
  // This prevents contribution/payment history rows from being displayed as active groups.
  return Boolean(
    value.role !== undefined ||
      value.is_active !== undefined ||
      value.joined_at !== undefined ||
      value.group_name !== undefined ||
      (groupValue && typeof groupValue === "object" && groupValue.name !== undefined)
  );
}

export function getGroupIdFromMembership(m: GroupMembership): number | null {
  const g: any = m?.group;
  return toOptionalNumber(g) || toOptionalNumber(g?.id) || toOptionalNumber(m?.group_id);
}
...
export function getJoinRequestGroupId(r: GroupJoinRequest): number | null {
  const g: any = r?.group;
  return toOptionalNumber(g) || toOptionalNumber(g?.id) || toOptionalNumber(r?.group_id);
}
...
export async function listGroupMemberships(
  ...
  return asArray<GroupMembership>(res.data).filter(isGroupMembershipRow);
}
