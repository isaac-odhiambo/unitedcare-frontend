...
// - UN2      => group contribution for GROUP payment_code=UN and USER id 2
// ------------------------------------------------------
...
export function normalizeWithdrawalPhone(phone: string): string {
  const clean = String(phone || "").replace(/\D/g, "");
...
export function isAllocationDone(tx?: MpesaTransaction | null): boolean {
  return [
    ...
    "MANUAL_REVIEW",
    "INVALID_REFERENCE",
  ].includes(String(tx?.allocation_status || "").toUpperCase());
}
...
/**
 * Group-id reference style supported by backend:
 * - grp9
 * - group9
 * - GROUP-9
 *
 * We standardize frontend to grp{id}.
 */
...
/**
 * Group payment_code + user_id style supported by backend:
 * - UN2
 * - WF12
 */
export function buildGroupCodeUserReference(
  paymentCode: string,
  userId: number | string
) {
  return `${String(paymentCode || "").trim()}${Number(userId)}`.toUpperCase();
}
...
export async function requestWithdrawal(
  ...
  const body = {
    phone: normalizeWithdrawalPhone(payload.phone),
    amount: normalizeAmount(payload.amount),
    source: payload.source ?? "SAVINGS",
  };

  console.log("PAYMENTS_SERVICE_WITHDRAWAL_BODY", body);

  const res = await api.post(ENDPOINTS.payments.requestWithdrawal, body);
  return res.data;
}
...
/**
 * Existing group-id flow, unchanged in behavior.
 * Only the reference format is standardized to grp{id},
...
/**
 * New payment_code + user_id flow supported by backend.
 * Example:
 * - paymentCode=UN, userId=2 => UN2
 * - paymentCode=WF, userId=12 => WF12
 */
export function stkContributeGroupByCode(
  phone: string,
  amount: string | number,
  paymentCode: string,
  userId: number
  ...
  return mpesaStkPush({
    ...
    amount,
    purpose: "GROUP_CONTRIBUTION",
    reference: buildGroupCodeUserReference(paymentCode, userId),
...
// /* ADJUSTED ONLY FOR WITHDRAWAL REQUESTS:
//    Backend serializer expects local format: 07XXXXXXXX or 01XXXXXXXX */
// export function normalizeWithdrawalPhone(phone: string): string {
//   const clean = String(phone || "").replace(/\D/g, "");
...
// /**
//  * Pending means allocation is still not in any terminal state.
//  * We keep only UNALLOCATED as truly pending here so that terminal
//  * allocation outcomes like MANUAL_REVIEW / INVALID_REFERENCE do not
//  * leave the payment UI stuck forever.
//  */
// export function isAllocationPending(tx?: MpesaTransaction | null): boolean {
//   return ["UNALLOCATED"].includes(
//     String(tx?.allocation_status || "").toUpperCase()
...
// /**
//  * Done means allocation has reached a terminal state.
//  * This matches deposit.tsx terminal allocation logic.
//  */
// export function isAllocationDone(tx?: MpesaTransaction | null): boolean {
//   return [
//     "AUTO_ALLOCATED",
