...
function parseBackendError(e: any): FieldErrors {
  ...
  if (data?.username) errors.username = String(data.username);
  if (data?.phone) errors.phone = String(data.phone);
  if (data?.email) errors.email = String(data.email);
  if (data?.password) errors.password = String(data.password);
...
export default function RegisterScreen() {
  ...
  const validate = (): boolean => {
    ...
    if (!cleanName) next.username = "Name is required.";

    if (!cleanPhone || !/^(07|01)\d{8}$/.test(cleanPhone)) {
