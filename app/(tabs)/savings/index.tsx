...
const TEXT_ON_DARK_SOFT = "rgba(255,255,255,0.72)";
...
function getUserId(user: any): number | null {
  const raw = user?.id ?? user?.user_id ?? user?.pk ?? null;
  const n = Number(raw);
  ...
  return (
    <View style={styles.infoCard}>
      <View style={styles.infoIcon}>
        <Ionicons name={icon} size={18} color="#8CF0C7" />
      </View>
...
function ActionItem({ title, icon, onPress, disabled }: ActionItemProps) {
  return (
    <TouchableOpacity
      activeOpacity={0.86}
      onPress={onPress}
    ...
    >
      <Ionicons
        name={icon}
        size={20}
        color={disabled ? TEXT_ON_DARK_MUTED : WHITE}
      />
...
export default function SavingsIndexScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const savingsAccount = await getOrCreateDefaultSavingsAccount();
  ...
  const accountName = account?.name || "Rescue Plan";
  ...
  const availableBalance = useMemo(
    () => formatKes(account?.available_balance ?? account?.balance),
  ...
  const displayAvailable = loading && !account ? "KES 0.00" : availableBalance;
  const displayTotal = loading && !account ? "KES 0.00" : totalBalance;
  const displayReserved = loading && !account ? "KES 0.00" : reservedAmount;
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
      ...
      >
        ...
        <View style={styles.balanceCard}>
          ...
          <Text style={styles.planName}>{accountName}</Text>

          <Text style={styles.balanceLabel}>Available balance</Text>
          <Text style={styles.balanceValue}>{displayAvailable}</Text>
        ...
        {error ? (
          <View style={styles.errorCard}>
            <Ionicons name="alert-circle-outline" size={19} color="#FCA5A5" />
            <Text style={styles.errorText}>{error}</Text>
          </View>
        ) : null}

        <View style={styles.summaryWrap}>
          <InfoCard label="Total saved" value={displayTotal} icon="cash-outline" />

          <InfoCard
            label="Reserved"
            value={displayReserved}
        ...
        {loading && !account ? (
          <Text style={styles.silentLoadingText}>Updating savings...</Text>
        ) : null}
      </ScrollView>
    </SafeAreaView>
  );
