...
function MyMerryCard({
  ...
  const showCurrentDueState = currentTotal > 0 && !fullyCoveredByWallet;
  ...
  const merryCardStyle = showOverdueState
    ? styles.merryCardDanger
    : showCurrentDueState
      ? styles.merryCardWarning
      : fullyCoveredByWallet
        ? styles.merryCardSuccess
        : styles.merryCard;
  ...
  return (
    <Card style={merryCardStyle} variant="default">
      ...
      {remainingAfterWallet > 0 ? (
        <Text style={styles.helperTextStrong}>
          This merry needs your attention.
        </Text>
      ) : null}
...
export default function MerryIndexScreen() {
  ...
  const summaryQuery = useQuery<MyAllMerryDueSummaryResponse | null>({
    queryKey: ["merry", "all-due-summary"],
    queryFn: async () => getMyAllMerryDueSummary(),
  });

  const availableQuery = useQuery<AvailableMerryRow[]>({
    queryKey: ["merry", "available"],
    queryFn: async () => (await getAvailableMerries()) ?? [],
  });

  const summary = summaryQuery.data ?? null;
  const availableMerries = availableQuery.data ?? [];

  const loading =
    summaryQuery.isLoading || availableQuery.isLoading;

  const refreshing =
    summaryQuery.isRefetching || availableQuery.isRefetching;
    ...
    if (availableQuery.error) {
      errors.push(
        getApiErrorMessage(availableQuery.error) ||
          getErrorMessage(availableQuery.error)
      );
    }

    return errors.filter(Boolean).join(" • ");
  }, [availableQuery.error, summaryQuery.error]);
  ...
  const attentionCount = useMemo(() => {
    const directCount = Number((summary as any)?.attention_count ?? (summary as any)?.unread_count);
    if (Number.isFinite(directCount) && directCount > 0) {
      return Math.min(999, Math.floor(directCount));
    }

    return myGroupsPreview.filter((item) => {
      const dueNow = moneyNumber(item.total_due_now);
      const wallet = moneyNumber(item.wallet_balance);
      return Math.max(0, dueNow - wallet) > 0;
    }).length;
  }, [summary, myGroupsPreview]);
...
//   const userQuery = useQuery<MerryUser | null>({
//     queryKey: ["auth", "merry-index-user"],
//     queryFn: async () => {
...
//   const loading =
//     userQuery.isLoading || summaryQuery.isLoading || availableQuery.isLoading;
...
//       availableQuery.refetch(),
//     ]);
//   }, [availableQuery, summaryQuery, userQuery]);
