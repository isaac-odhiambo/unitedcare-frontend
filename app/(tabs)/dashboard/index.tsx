...
export default function DashboardScreen() {
  ...
  const openDirectGroupPaymentFlow = useCallback(async (groupItem?: any | null) => {
    ...
    try {
      ...
      const groupName = String(
        (loadedGroup as any)?.name || groupItem?.name || "Community space"
      ).trim();
  ...
  const openGroupContributionFlow = useCallback((groupId?: number | null) => {
    ...
    const selectedGroup =
      (selectedDashboardGroup && Number(selectedDashboardGroup.id) === Number(groupId)
        ? selectedDashboardGroup
        : null) ||
      activeContributionGroups.find(
        (item) => Number(item.id) === Number(groupId)
      ) ||
      null;

    if (selectedGroup) {
      openDirectGroupPaymentFlow(selectedGroup);
      return;
    }

    router.push(ROUTES.tabs.groups as any);
  }, [activeContributionGroups, openDirectGroupPaymentFlow, selectedDashboardGroup]);
  ...
  const spaceCards = useMemo(() => {
    ...
    if (hasActiveGroups) {
      cards.push({
        key: "groups",
        title: firstActiveGroupName || "Group",
        membersLabel:
          activeGroupMemberships.length === 1
            ? "1 active group"
            : `${activeGroupMemberships.length} active groups`,
        valueLabel: activeGroupMemberships.length === 1 ? "Open group" : "Open details",
        amountLabel: "Open",
        icon: "people-outline",
        tone: "groups",
        onPress: () =>
          firstActiveGroupId
            ? router.push(ROUTES.dynamic.groupDetail(firstActiveGroupId) as any)
            : router.push(ROUTES.tabs.groups as any),
      });
    }
        ...
        <View style={[styles.overviewGrid, isWideScreen && styles.overviewGridWide]}>
          ...
          <View style={[styles.overviewItem, isWideScreen && styles.overviewItemWide]}>
            <OverviewCard
              ...
              onPress={() =>
                selectedDashboardGroup
                  ? openDirectGroupPaymentFlow(selectedDashboardGroup)
                  : hasActiveGroups
                  ? router.push(ROUTES.tabs.groupsMemberships as any)
                  : router.push(ROUTES.tabs.groups as any)
              }
