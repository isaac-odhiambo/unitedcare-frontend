...
export const ROUTES = {
  ...
  dynamic: {
    /* =========================================================
       LOANS
       Actual file expected: app/(tabs)/loans/[id].tsx
    ========================================================= */
    loanDetail: (id: number | string) => `/(tabs)/loans/${id}`,

    loanPay: (loanId: number | string) =>
      `/(tabs)/loans/pay?loanId=${loanId}`,

    loanAddGuarantor: (loanId: number | string) =>
      `/(tabs)/loans/add-guarantor?loanId=${loanId}`,

    loanGuaranteeRequests: (loanId?: number | string) =>
      loanId
        ? `/(tabs)/loans/guarantee-requests?loanId=${loanId}`
        : "/(tabs)/loans/guarantee-requests",
    ...
    loanPaymentDeposit: (
      ...
      return (
        `/(tabs)/payments/deposit?source=loan` +
        `&purpose=LOAN_REPAYMENT` +
        `&loanId=${loanId}` +
        `&borrowerUserId=${borrowerUserId}` +
        amountParam +
        `&editableAmount=true` +
        `&returnTo=${encodeURIComponent(`/(tabs)/loans/${loanId}`)}`
      );
    },
    ...
    savingsDeposit: (accountId?: number | string, amount?: number | string) => {
      const accountParam =
      ...
      return `/(tabs)/savings/save${accountParam}${amountParam}`;
    },

    savingsWithdrawal: (accountId?: number | string, amount?: number | string) => {
      const accountParam =
        accountId === undefined || accountId === null || accountId === ""
          ? ""
          : `?accountId=${accountId}`;
      ...
      return `/(tabs)/payments/request-withdrawal${accountParam}${amountParam}`;
    },
  },
} as const;
